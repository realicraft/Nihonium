...
alt_minvers = {"nihonium2": versions.Version(0, 9, 0), "flerovium": versions.Version(1, 0, 0)} # Used to define minimum versions for other bots. Format: {"<id>": versions.Version(<version>)}
...
# Commands can take any number of placement arguments and should return a string (or discord.Embed) containing the output of the command. (Beginning/Trailing newline not required.)
...
def f_bot(bot_data, thread_data, user_data):
    import discord
...
def f_help(bot_data, thread_data):
    ...
    output.add_field(name="nh!suggest suggestion;str;allows_spaces",value="Make a suggestion.")
    output.add_field(name="nh!text command;str;no_spaces;'read' filename;str;no_spaces;'_' other;varies",value="Text file modificaton.")
    output.add_field(name="nh!{file|files} command;str;no_spaces;'read' filename;str;no_spaces;'_.txt' other;varies",value="File modificaton.")
...
ex_commands = {"nihonium2": {"coin2": coin2},"flerovium": {"bot": f_bot, "help": f_help}}